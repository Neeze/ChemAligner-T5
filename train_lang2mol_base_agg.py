...
from src.lightning_module_lang2mol_base_agg import T5Model
...
def main(args):
    ...
    train_dataloader = get_dataloaders(args, tokenizer, batch_size=args.batch_size, num_workers=args.num_workers, split='train', task='lang2mol')
    val_dataloader = get_dataloaders(args, tokenizer, batch_size=args.batch_size, num_workers=args.num_workers, split='validation', task='lang2mol')
    ...
    token_ids_to_use = [
        tid for tid in tokenizer.get_added_vocab().values()
        if tid >= 32120 and tid < 35075
    ]

    agg_args = Namespace(
        alpha=0.03,
        memory_len=args.train_data_len,
        token_ids_to_use=token_ids_to_use
    ) 

    model = T5Model(args, agg_args)
    model.to(device)
